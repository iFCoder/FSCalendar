...
public class SimpleMonthAdapter extends RecyclerView.Adapter<SimpleMonthAdapter.ViewHolder> implements SimpleMonthView.OnDayClickListener {
    ...
    public int getItemCount() {
        int itemCount = 5 * MONTHS_IN_YEAR;
    ...
    public void setSelectedDay(CalendarDay calendarDay) {
        if (selectedDays.getFirst() != null && selectedDays.getLast() == null) {
            if ((selectedDays.getFirst().getDay() >= calendarDay.day) && (selectedDays.getFirst().getMonth() >= calendarDay.month) && (selectedDays.getFirst().getYear() >= calendarDay.year)) {
