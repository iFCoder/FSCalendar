...
class SimpleMonthView extends View {
    ...
    protected static int MONTH_DAY_LABEL_TEXT_SIZE;  //周字体大小
    ...
    public SimpleMonthView(Context context, TypedArray typedArray) {
        ...
        mDayNumColor = typedArray.getColor(R.styleable.DayPickerView_colorNormalDay, resources.getColor(R.color.normal_day));  //中间颜色值
        ...
        mMonthTitleBGColor = typedArray.getColor(R.styleable.DayPickerView_colorSelectedDayText, resources.getColor(R.color.selected_day_text));  //点击选中背景颜色
        mWeekendsColor = resources.getColor(R.color.normal_day);  //两边颜色值
        mStartEndColor = resources.getColor(R.color.start_end_text_color);
        ...
        while (day <= mNumCells) {
            ...
            if (mMonth == mSelectedBeginMonth && mSelectedBeginDay == day && mSelectedBeginYear == mYear) {
                ...
                if (mHasToday && (mToday == day)) {
                    ...
                    if (day == 1) {
                        drawMonthTitle(canvas);  //在每个月1号的上面画当前月份
                    }
    ...
    protected void drawMonthNums_(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            if ((mMonth == mSelectedBeginMonth && mSelectedBeginDay == day && mSelectedBeginYear == mYear) || (mMonth == mSelectedLastMonth && mSelectedLastDay == day && mSelectedLastYear == mYear)) {
                drawRoundRect(mDrawRect, x, y, canvas);

                if ((mSelectedBeginDay != -1 && mSelectedBeginMonth != -1 && mSelectedBeginYear != -1) &&
                        (mSelectedLastDay == -1 && mSelectedLastMonth == -1 && mSelectedLastYear == -1)) {
                    fDay = mSelectedBeginDay;
                    fYear = mSelectedBeginYear;
                    fMonth = mSelectedBeginMonth;
                    canvas.drawText("开始", x + 2, getTextYCenter(mStartEndPaint, y - (MINI_DAY_NUMBER_TEXT_SIZE / 3) + 30), mStartEndPaint);
                } else {
                    if (fDay == day && fMonth == mMonth && fYear == mYear) {
                        canvas.drawText("开始", x + 2, getTextYCenter(mStartEndPaint, y - (MINI_DAY_NUMBER_TEXT_SIZE / 3) + 30), mStartEndPaint);
                    } else {
                        canvas.drawText("结束", x + 2, getTextYCenter(mStartEndPaint, y - (MINI_DAY_NUMBER_TEXT_SIZE / 3) + 30), mStartEndPaint);
                    }
                }
     ...
     * @param drawRect
     */
    private void drawRoundRect(Boolean drawRect, int x, int y, Canvas canvas) {
        if (drawRect) {
            RectF rectF = new RectF(x - DAY_SELECTED_CIRCLE_SIZE, (y - MINI_DAY_NUMBER_TEXT_SIZE / 3) - DAY_SELECTED_CIRCLE_SIZE, x + DAY_SELECTED_CIRCLE_SIZE, (y - MINI_DAY_NUMBER_TEXT_SIZE / 3) + DAY_SELECTED_CIRCLE_SIZE);
            canvas.drawRoundRect(rectF, 10.0f, 10.0f, mSelectedCirclePaint);
        } else {
